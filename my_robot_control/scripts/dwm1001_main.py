...
serialPortDWM1001 = serial.Serial(
    port       = str(rospy.get_param('~serial_port_name1', '/dev/ttyACM0')),
...
s
        ...
        global x1, y1, x2, y2, en1  ,en2, i, s, en_pub
        global Ready 
        Ready = False
        en_pub=0
        ...
        a = rospy.Subscriber("UWB_yaw", Bool, callback=sub_callback)
        try:
            ...
            while not rospy.is_shutdown():
                ...
                try:
                    ...
                    if en1 > 0 and en2 > 0:
                        if xt!=0 or yt!=0: 
                            if Ready == True:   
                                yaw = math.atan2(yt,xt)
                                
                                rospy.loginfo("hoanthanh")
                              
                                position = Vector3(float((x1+x2)/2),float((y1+y2)/2),yaw)
                                pub_po=rospy.Publisher('/dwm1001/position', Vector3, queue_size=1)
                                pub_po.publish(position)
                                x1=x2=en1=en2=0
                            else:
                                rospy.loginfo("False")
                                i=s=en_pub=0
    ...
    #             en1=en1+1
    #             # publish tag
    #             pub_tag1 = rospy.Publisher('/dwm1001/tag1', Tag, queue_size=1)
    #             pub_tag1.publish(tag1)
    #             x1=tag1.x
    #             y1=tag1.y
    #             rospy.loginfo("Tag1: "
    #                           + " x: "
    ...
    def pubblishCoordinatesIntoTopics1(self, networkDataArray):
        ...
        for network in networkDataArray:
            ...
            if 'AN'.encode() in network:
                ...
                # construct the object for the tag
                tag1 = Tag(float(networkDataArray[networkDataArray.index(network) + 1]),
                          float(networkDataArray[networkDataArray.index(network) + 2]),
                          float(networkDataArray[networkDataArray.index(network) + 3]),)
                en1=en1+1
                # publish tag
    ...
    def pubblishCoordinatesIntoTopics2(self, networkDataArray):
        ...
        for network in networkDataArray:
            ...
            if 'AN'.encode() in network:
                ...
                # construct the object for the tag
                tag2 = Tag(float(networkDataArray[networkDataArray.index(network) + 1]),
                          float(networkDataArray[networkDataArray.index(network) + 2]),
                          float(networkDataArray[networkDataArray.index(network) + 3]),)
                en2=en2+1
                # publish tag
                pub_tag2 = rospy.Publisher('/dwm1001/tag2', Tag, queue_size=1)
                pub_tag2.publish(tag2)
                x2=tag2.x
                y2=tag2.y
                rospy.loginfo("Tag2: "
                              + " x: "
